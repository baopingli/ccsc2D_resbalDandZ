...
[~,files] = split_folders_files(subdir);%返回文件夹和文件
    ...
    for file=image_frames{1}:image_frames{2}:image_frames{3}
        ...
        if (files(file).isdir == 0)
            ...
            try
                ...
                if(actual_files==1)
                    I = cell(1,length(files));%创建元胞用来存图像。
                end
                I{actual_files} = IMG;
...
elseif(ischar(imgs_path) && (length(files)==1 && strcmp(files(1).name(end-3:end),'.mat'))) % Only 1 file in folder and it's a .mat
    ...
    clear imgs_path
    % Make sure the images are single.单精度
    I = single(I);
    
    
    % Select the ones you want.
    eval(strcat('I = I(:,:,:,',last_ind,');'));%将字符串转换为matlab可执行语句。
    origI = I;
    I = cell(1,size(origI,4));%整成1行N列的形式
    ...
    clear origI
elseif(ischar(imgs_path) && exist(imgs_path,'file')~=0)  % Path is to a file with all images in it.所有图片所在的文件夹
    fprintf('\nLoading %s\n',imgs_path);
    % May be a .mat file.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Convert the colors here. 首先进行颜色的转换
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% old_I = I;
for i=1:length(I)
    switch(COLOR_TYPE)
